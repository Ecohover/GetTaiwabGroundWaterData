...
namespace GetData
{
    class RLStation
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(RLStation));
        ...
        public void GetYearsList()
        {
            ...
            try
            {
                ...
                {
                    node = doc.SelectSingleNode("result");
                    strYearsList = node.Attributes["DqvCnt"].Value;
                    foreach (var item in strYearsList.Split(','))
                    {
                        listYear.Add(item);
                    }
                }
